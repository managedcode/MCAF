...
description: Create, update, and validate repository skills under `skills/` (Agent Skills format) so they match the real codebase and `AGENTS.md` rules. Tune YAML `description` triggers, apply user feedback to skills, and generate `<available_skills>` metadata blocks for agent runtimes.
...
- New/updated skill folders under `skills/` with valid `SKILL.md` frontmatter and lean workflows
...
5. Validate skills (fix errors before shipping):
   - run: `python3 skills/mcaf-skill-curation/scripts/validate_skills.py skills`
6. Generate a metadata-only skills block for your agent runtime:
   - run: `python3 skills/mcaf-skill-curation/scripts/generate_available_skills.py skills`
   - paste the output into your agent configuration (system/developer prompt)
7. When user feedback is about skills:
   - update the relevant `skills/<skill-name>/SKILL.md` (especially YAML `description` triggers) so the fix is permanent

## Bundled scripts

- `scripts/validate_skills.py` — validates frontmatter + folder/name rules and flags common spec violations.
- `scripts/generate_available_skills.py` — prints an `<available_skills>` XML block from `skills/*/SKILL.md` metadata.
